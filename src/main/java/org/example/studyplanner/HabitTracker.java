...
public class HabitTracker {
    ...
    public int addHabit(HabitDetails details) {
        Habit habit = new Habit(details.getName(), details.getMotivation(), details.getDuration(), this.nextId, details.getStartDate(), details.isConcluded());
    ...
    public int handleAddHabitAdapter(List<String> stringProperties, List<Integer> intProperties, boolean isConcluded){
        LocalTime lt = LocalTime.of(intProperties.get(1), intProperties.get(0)); // Swap order for minutes, hours
        ...
        HabitDetails details = new HabitDetails(stringProperties.get(0), stringProperties.get(1), lt, startDate, isConcluded);
        return addHabit(details);
    }
