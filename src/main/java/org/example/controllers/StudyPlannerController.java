...
public class StudyPlannerController {
    ...
    private void handleAddHabit() {
        ...
        // Use HabitBuilder to construct the habit
        HabitTracker.HabitBuilder builder = new HabitTracker.HabitBuilder(habitTracker.getTrackerKeys().size() + 1)
                .withName(name)
                .withMotivation(motivation)
                .withDailyDedicationTime(dailyHoursDedication, dailyMinutesDedication)
                .withStartDate(start.getYear(), start.getMonthValue(), start.getDayOfMonth(), start.getHour(), start.getMinute(), start.getSecond())
                .withIsConcluded(false);

        // Add the habit using the builder
